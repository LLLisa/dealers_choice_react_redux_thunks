...
import { addCompany, removeCompany } from '../store/company-store';

class CompanyList extends React.Component {
  render() {
    return (
      <div>
        <button onClick={this.props.addOne}>Add Company</button>
        {this.props.companies.map((company, i) => {
          return (
            <li key={i}>
              <span>{company.name}</span>
              <span>
                <button onClick={() => this.props.removeOne(company)}>
                  remove company
                </button>
              </span>
